...
load_dotenv()  # Load environment variables from .env before settings are read
...
async def lifespan(app: FastAPI):
    ...
    # Auto-migrate admin_questions columns
    try:
        from core.database import db_manager
        from sqlalchemy import text as _text
        async with db_manager.async_session_maker() as _session:
            for _col, _type in [("config_json","TEXT"),("visual_type","TEXT"),("visual_data","TEXT"),("misconception_type","TEXT"),("retention_tag","TEXT")]:
                await _session.execute(_text(f"ALTER TABLE admin_questions ADD COLUMN IF NOT EXISTS {_col} {_type};"))
            await _session.commit()
        logger.info("admin_questions columns migration completed")
    except Exception as _e:
        logger.warning(f"admin_questions migration skipped: {_e}")

    # Multi-country architecture migration - add country_id and curriculum_id to content tables
    try:
        from core.database import db_manager
        from sqlalchemy import text as _text
        async with db_manager.async_session_maker() as _session:
            # Add country_id and curriculum_id to grades
            await _session.execute(_text("ALTER TABLE grades ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE grades ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            # Add country_id and curriculum_id to semesters
            await _session.execute(_text("ALTER TABLE semesters ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE semesters ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            # Add country_id and curriculum_id to admin_subjects
            await _session.execute(_text("ALTER TABLE admin_subjects ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE admin_subjects ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            # Add country_id and curriculum_id to admin_skills
            await _session.execute(_text("ALTER TABLE admin_skills ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE admin_skills ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            # Add country_id and curriculum_id to admin_questions
            await _session.execute(_text("ALTER TABLE admin_questions ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE admin_questions ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            # Add country_id and curriculum_id to interactions
            await _session.execute(_text("ALTER TABLE interactions ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE interactions ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            # Add country_id and curriculum_id to units
            await _session.execute(_text("ALTER TABLE units ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE units ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            # Add country_id and curriculum_id to admin_lessons
            await _session.execute(_text("ALTER TABLE admin_lessons ADD COLUMN IF NOT EXISTS country_id INTEGER;"))
            await _session.execute(_text("ALTER TABLE admin_lessons ADD COLUMN IF NOT EXISTS curriculum_id INTEGER;"))
            await _session.commit()
        logger.info("Multi-country architecture migration completed")
    except Exception as _e:
        logger.warning(f"Multi-country migration skipped: {_e}")
    # MODULE_STARTUP_END
